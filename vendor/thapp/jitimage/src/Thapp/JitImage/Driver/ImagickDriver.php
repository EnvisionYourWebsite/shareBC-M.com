...
class ImagickDriver extends ImDriver
{
     ...
     * scaling
     *
     * @var Thapp\JitImage\Driver\Scalling
     */
    protected $scaling;= sys_get_temp_dir();
        $this->loader  = $loader;
    ...
    {
        extract($this->scaling->getCropCoordinates(
    ...
    protected function resize($width, $height, $flag = '')
    {
        ...
        case static::FL_FILL_AREA:
            $this->scaling->fillArea($width, $height, $this->getInfo('width'), $this->getInfo('height'));
            break;
        // ignoring aspect ration is default behaviour on imagick resize
        case static::FL_IGNR_ASPR:
            break;
        case static::FL_PIXL_CLMT:
            extract($this->scaling->pixelLimit($this->getInfo('width'), $this->getInfo('height'), $width));
            break;
        case static::FL_RESZ_PERC:
            extract($this->scaling->percentualScale($this->getInfo('width'), $this->getInfo('height'), $width));
            break;
        ...
        case static::FL_OSRK_LGR:
            extract($this->scaling->fitInBounds($width, $height, $this->getInfo('width'), $this->getInfo('height')));
            break;
    ...
    protected function getSourceAttributes()
    {
        ...
        return [
            'width'  => $width,
            'height' => $height,
            'ratio'  => $this->scaling->ratio($width, $height),
