...
class ImDriver extends AbstractDriver
{
     ...
     * scaling
     *
     * @var Thapp\JitImage\Driver\Scaling
     */
    protected $scaling;
     ...
     * ShellCommand
     *
     * @var Thapp\JitImage\Shell\ShellCommand
     */
    ...
    public function process()
    {
        ...
        $this->shellCommand->runCmd($cmd, '\Thapp\JitImage\Exception\ImageProcessException',
            function ($stderr)
            {
                $this->clean();
            },
     ...
     * @return boolean|void
     */
    public function filter($name, array $options = array())
    {
    ...
    {
        $this->commands['-quality %d'] = array((int)$quality);
    }
    ...
    {
        if (!is_null($color)) {
            $this->commands['-background "#%s"'] = array(trim($color, '#'));
        }
        return $this;
    }
    ...
    protected function resize($width, $height, $flag = '')
    {
        ...
        $this->commands[$cmd] = array($w, $h, $flag);
        return $this;
    }
    ...
    {
        $this->commands['-extent %sx%s%s'] = array((string)$width, (string)$height, $flag);
    ...
    {
        $this->commands['-gravity %s%s'] = array($this->getGravityValue($gravity), $flag);
    ...
    {
        $this->commands['-scale %s%s%s'] = array($width, $height, $flag = '');
    ...
    protected function isMultipartImage()
    {
        if (!is_int($this->imageFrames)) {
            ...
            if ('image/gif' !== $type and 'image/png' !== $type) {
                ...
                $this->imageFrames = (int)$this->shellCommand->runCmd($cmd, '\Thapp\JitImage\Exception\ImageProcessException',
                    function ($stderr)
                    {
                        $this->clean();
                    },
