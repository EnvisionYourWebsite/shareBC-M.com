...
class Scaling
{
     ...
     * @return mixed
     */
    public function fitInBounds($w, $h, $width, $height)
    {
        if ($width >= $w or $height >= $h) {
            ...
            list($width, $height) = $width <= $height ? array($valW, $isR ? $h : $valH) : array($isR ? $w : $valW, $valH);
        }
     ...
     * @return void
     */
    public function fillArea(&$w, &$h, $width, $height)
    {
        ...
        list($w, $h) = $ratio > $ratioC ? array((int)ceil($h * $ratio), $h) : array($w, (int)ceil($w / $ratio));
    }
     ...
     */
    public function pixelLimit($width, $height, $limit)
    {
        $ratio  = $this->ratio($width, $height);
     ...
     * @return array $width and $height
     */
    public function percentualScale($width, $height, $percent)
    {
        $ratio  = $this->ratio($width, $height);
     ...
     */
    public function getCropCoordinates($width, $height, $w, $h, $gravity)
    {
        $x = $y = 0;
