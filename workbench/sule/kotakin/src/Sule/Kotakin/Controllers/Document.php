...
class Document extends BaseUser
{
    /**
     * The JitImage.
     *
    ...
    {
        if ( ! $this->isAuthor($term)) {
            Session::flash('error', $this->getUtility()->t('You are not the uploader of this file.'));
    ...
    public function delete($slug)
    {
        ...
        $term = $this->getKotakin()->getTerm()->where('object_id', '=', $doc->getId())
                    ->where('is_file', '=', 1)
                    ->where('author_id', '=', $user->getId())->first();
        ...
        Session::flash('success', sprintf($this->getUtility()->t('File "%s" successfully deleted.'), $fileName));
    }
    ...
    )
    {
        ...
        if ( ! $this->isFolderStillExist($termId)) {
            $error = $this->getUtility()->t('The folder might be deleted by you or Administrator.');
            return $this->createResponse($defaultData, $error);
        }

        if ( ! $this->isFolderSharedToThisUser($this->term)) {
            $error = $this->getUtility()->t('You does not having access anymore to the folder.');
            return $this->createResponse($defaultData, $error);
        }

        if ( ! $this->isUserAllowedToUploadToThisFolder($this->term)) {
            $error = $this->getUtility()->t('Uploading file is disabled in this folder.');
            return $this->createResponse($defaultData, $error);
        }
        ...
        try {
            ...
            // $thumbWidth = $this->getKotakin()->config('thumb_width');
            // $thumbHeight = $this->getKotakin()->config('thumb_height');
            // $this->getJitImage()->source($path.'/'.$fileName)->fit($thumbWidth, $thumbHeight);
        } catch (FileException $e) {
            $error = sprintf($this->getUtility()->t('Unable to process your uploaded file "%s" with following error: %s.'), $originalFileName, $e->getMessage());
            return $this->createResponse($defaultData, $error);
        }
        ...
        $defaultData['id']            = $term->getId();
        $defaultData['url']           = URL::to('/'.$this->getSlug().'/file/'.$doc->getAttribute('slug').'?dl=1');
        $defaultData['thumbnail_url'] = URL::to($fileTemplate->thumb('50x50')->permalink());
        $defaultData['delete_url']    = URL::to('/'.$this->getSlug().'/file/'.$doc->getAttribute('slug'));
    ...
    /**
     * Check if current user allowed to upload to this folder
     *
     * @param Sule\Kotakin\Models\TermInterface $term
     * @return bool
     */
    protected function isUserAllowedToUploadToThisFolder(TermInterface $term)
    {
        $folder = $this->getKotakin()->getFolder()
                        ->find($term->getAttribute('object_id'));

        if ( ! is_null($folder)) {
            return (bool) $folder->getAttribute('user_upload');
        }
    ...
    /**
     * Check if the user author of current file
     *
     * @param Sule\Kotakin\Models\TermInterface $term
     * @return bool
     */
    protected function isAuthor(TermInterface $term)
    {
        return ($this->getUser()->getId() == $term->getAttribute('author_id'));
    }
